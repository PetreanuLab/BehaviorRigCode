...
%     SubHeaderParam(obj, 'ChoiceSection', 'Choice Section', x, y); next_row(y,2);
    ...
    SoloParamHandle(obj, 'premature_cout_go_signal');
...
%     premature_cout.value = [];
    ...
    premature_cout_go_signal.value = zeros (1000,2);
%     too_late.value = [];
...
%     error_conc.value = zeros (1,2);
    ...
    SoloFunctionAddVars('PlotSection', 'ro_args', {'premature_cout', 'premature_cout_go_signal', ...
...
% %         disp(value(odorSamplingTime2))
    ...
    if (~isempty(parsed_events.states.premature_cout) == 1),
...
%         disp(value(premature_cout_conc));
    elseif (~isempty(parsed_events.states.premature_cout_go_signal) == 1),
        premature_cout_go_signal(n_started_trials,:) = [n_started_trials value(sideList(n_started_trials))];
%         premature_cout_conc.value = [value(premature_cout_conc); n_started_trials value(nextConcVector(n_started_trials))];
%         disp(value(premature_cout_conc));
...
%         rightPortWater.value = value(rightPortWater) + 1;
        ...
        if (~isempty (parsed_events.states.waiting_for_both_pokes) == 1)
            odorValveOn(1, n_started_trials) = (parsed_events.states.cin_odor_min(1));  
            validCpkOut(1, n_started_trials) = (parsed_events.states.waiting_for_both_pokes(1));
            odorSamplingTimeVector(1, n_started_trials) = ...
                value(validCpkOut(1, n_started_trials)) - value(odorValveOn(1, n_started_trials));
...
%     disp(value(odorSamplingTime2));
        ...
        if ~isempty (parsed_events.states.left_poke_in_correct) == 1
            ...
            movementTimeVector(1, n_started_trials) = ...
                value(validLpkIn(1, n_started_trials)) - value(validCpkOut(1, n_started_trials));
        elseif ~isempty ((parsed_events.states.left_poke_in_error)) == 1
            validLpkIn(1, n_started_trials) = (parsed_events.states.left_poke_in_error(1));
            movementTimeVector(1, n_started_trials) = value(validLpkIn(1,n_started_trials)) - value(validCpkOut(1,n_started_trials));
        elseif ~isempty ((parsed_events.states.right_poke_in_correct)) == 1
            ...
            movementTimeVector(1, n_started_trials) = value(validLpkIn(1,n_started_trials)) - value(validCpkOut(1,n_started_trials));
        elseif ~isempty ((parsed_events.states.right_poke_in_error)) == 1
...
%         disp(value(movementTimeVector(1, n_started_trials)))
        ...
        if (~isempty (parsed_events.states.left_poke_in_correct) == 1 || ...
            ...
            if value(currentOdor(1,n_started_trials)) == 1
                movementTimeOdor1Vector(1,n_started_trials) = value(movementTimeVector(1, n_started_trials));
            elseif value(currentOdor(1,n_started_trials)) == 2
                movementTimeOdor2Vector(1,n_started_trials) = value(movementTimeVector(1, n_started_trials));
            elseif value(currentOdor(1,n_started_trials)) == 3
                movementTimeMix1Vector(1,n_started_trials) = value(movementTimeVector(1, n_started_trials));
            elseif value(currentOdor(1,n_started_trials)) == 4
                movementTimeMix2Vector(1,n_started_trials) = value(movementTimeVector(1, n_started_trials));
            end
        end
                
        
%         disp(value(odorSamplingTimeOdor1Vector))
