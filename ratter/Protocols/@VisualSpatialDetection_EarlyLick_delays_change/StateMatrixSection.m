...
switch action
    ...
    case 'next_trial',
        ...
        if validTrial % If current trial is is attended
            ...
            if value(setRespWindowInV)
                ...
                if  value(rewardWitholding)                 % In case we reward the animals for WITHOLDING
                    sma = add_state(sma, 'name', 'response_window','self_timer', responseWindow,...
                        'input_to_statechange', {lick,'wrong_choice','Tup', 'stimulus_off_invalid'});
                    sma = add_state(sma, 'name', 'stimulus_off_invalid','self_timer', value(rwdWithDelay)+0.5 ,... % leave the stimulus on for 1-2 secs
                        'output_actions', {'SchedWaveTrig','stop_stim_wave'},...
                        'input_to_statechange', {'Tup', 'correct_invalid'});
       ...
       preStimulusOffTime =  1.5 + rand*0.5; % for training so that stimulus doesnt go off immediately with the lick
    %    preStimulusOffTime =  0.5; % for training so that stimulus doesnt go off immediately with the lick
       ...
       sma = add_state(sma, 'name', 'pre_stimulus_off','self_timer',preStimulusOffTime,... % NOTE this changes the stimulus off time for correct trials
        ...
        if 0 %  Use trial time, don't allow corrects to speed up trials
            sma = add_state(sma, 'name', 'valid_stimulus_off','self_timer', trial_length/2 ,... % leave the stimulus on for 1-2 secs
                'output_actions', {'SchedWaveTrig','stop_stim_wave'},...
                'input_to_statechange', {'trial_timer_In', 'pre_iti',...
                'trial_EARLY_LICKS_timer_In', 'pre_iti','Tup', 'pre_iti'});
        ...
        % Correct choice state
        % Opens valve, informs toolbox and waits trial duration before going to ITI
        if value(freeWaterInValid)
            sma = add_state(sma, 'name', 'correct_invalid','self_timer', 0.75,...
                'input_to_statechange', {'Tup', 'reward_state_invalid'});
            
            sma = add_state(sma, 'name', 'reward_state_invalid','self_timer', trial_length/2,...
                'output_actions', {'SchedWaveTrig','reward_delivery'},...
                'input_to_statechange', {'trial_timer_In', 'pre_iti',...
                'trial_EARLY_LICKS_timer_In', 'pre_iti','Tup', 'pre_iti'});
            
        else  % wait for a lick to reward with water
            sma = add_state(sma, 'name', 'correct_invalid','self_timer', responseWindow,...
                'input_to_statechange', {lick, 'reward_state_invalid','Tup', 'pre_iti'});
