...
switch action
    ...
    case 'next_trial',
        ...
        sma = add_scheduled_wave(sma,'name','wrong_timer',...
            'preamble',0.2,...    % this duration is decoded by the stimulus computer to present Visual Error or not
            'trigger_on_up', 'lick_pulse');
...
%             'trigger_on_up', 'stimulus_trigger');
        ...
        if value(setRespWindow)
            sma = add_state(sma, 'name', 'missed_response','self_timer', stim_length-responseWindow-changeStimDelay,...
        ...
        if  value(punishError)  % In case we punish the animals for making a mistake
            ...
            sma = add_state(sma, 'name', 'wrong_choice','self_timer', 10,...
                ...
                'input_to_statechange', {'trial_timer_In', 'wrongChoice_timeout','Tup', 'wrongChoice_timeout'}); % there is a hack here the Tup state change.. (is is possilbe that trial timer IN has already happened in this stage?
            
            sma = add_state(sma, 'name', 'wrongChoice_timeout','self_timer', value(errorTimeOut),...
                'input_to_statechange', {'Tup', 'pre_iti'});
        else
            sma = add_state(sma, 'name', 'wrong_choice','self_timer', 10,...
                'input_to_statechange', {'trial_timer_In', 'pre_iti','Tup', 'pre_iti'});
        ...
        sma = add_state(sma, 'name', 'early_choice','self_timer', 0.001,...
            'input_to_statechange', {'Tup', 'wrong_choice'});
