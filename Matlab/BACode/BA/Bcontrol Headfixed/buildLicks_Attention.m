function licks = buildLicks_Attention(dp)
...
for itrial   = 1:sweeps.ntrials
    ...
    thisTrialLicks = (dp.parsedEvents_history{itrial}.pokes.L + timeOffset)*1000;
    if ~isempty(thisTrialLicks)
        if itrial < sweeps.ntrials-1
            nextTrialLicks = (dp.parsedEvents_history{itrial+1}.pokes.L + timeOffset)*1000;
        end
        ...
        delta = thisTrialLicks(:,1) - sweeps.timeStimulusChange(itrial);
        ...
        if ~isempty(thisTrialLicks)
            ...
            % nLicksBeforeStimulusChange
            b =  thisTrialLicks(:,1) < sweeps.timeStimulusChange(itrial);
            if any(b),sweeps.nLicksBeforeStimulusChange(itrial) =sum(b);
            end
