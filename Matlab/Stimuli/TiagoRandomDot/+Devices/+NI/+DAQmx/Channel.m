classdef Channel < Devices.NI.DAQmx.private.DAQmxClass
    ...
    %Leave these to subclasses
    
    
    %% PROP ACCESS METHODS
    methods
        function regExp = get.gsPropRegExp(obj)
            regExp = ['.*DAQmxGet' obj.typeCode '(.*)\(ulong,\s*cstring,\s*(\S*)[\),].*'];
        end
    ...
    methods (Hidden, Access=protected)
        function obj = createChannels(obj, createFunc, task, deviceNames, physChanIDs, chanNames, varargin)
            % Constructor-like method meant to be invoked by subclasses to universally handle their construction logic. This is the de facto constructor.
            ...
            %%%TMW: This method is implemented as a method in an abstract superclass, to allow it to be inherited by subclasses.
            % It was not possible to implement this logic directly in the superclass constructor (for the concrete subclasses to inherit), because
            % the case of creating an object array required then either 1) conditionally calling that constructor from subclass, or 2) constructing abstract class (as would be done if this logic were shared in superclass constructor).
            % In principle, it would be preferable for this shared construction logic to be implemented in the constructor, if one or the other restriction could be relaxed.
            ...
            elseif ~strcmpi(obj.type,task.taskType)
                fprintf(2,'WARNING: Cannot add Channels of differing types to Task\n');
                delete(obj);
                return;
            end
            ...
            for i = 1:length(deviceNames)
                for j = 1:length(chanNameArray{i})
                    ...
                    %Create the channel!
                    try
                        obj(count).driverCall(createFunc,task.taskID, physChanNameArray{i}{j}, chanNameArray{i}{j}, varargin{:});
                    catch ME
                    ...
                    actChanNames = textscan(actChanNames,'%s');
                    ...
                    obj(count).device = Devices.NI.DAQmx.Device(deviceNames{i});
